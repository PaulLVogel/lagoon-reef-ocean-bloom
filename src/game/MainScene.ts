...
export class MainScene extends Phaser.Scene {
  ...
  private isBossWave(wave = this.wave) { return wave > 0 && wave % BOSS_WAVE === 0; }
  private eraIndex(wave = this.wave) { return Math.max(0, Math.floor((wave - 1) / BOSS_WAVE)); }
  private eraHordeMul(wave = this.wave) { return Math.pow(ERA_HORDE_MUL, this.eraIndex(wave)); }
  ...
  private detonateMortar(s: MortarShell) {
    ...
    blast.setStrokeStyle(3, 0xbbf7d0, 0.9); blast.setDepth(16);
    this.tweens.add({ targets: blast, scale: Math.max(1, s.aoe / 8), alpha: 0, duration: 280, ease: "Cubic.easeOut", onComplete: () => blast.destroy() });
    for (const e of this.enemies) if (e.alive && e.overlaps(x, y, s.aoe)) this.applyEnemyHit(e, s.damage);
  }
  private clearMortars() { for (const s of this.mortars) s.gfx.destroy(); this.mortars = []; }
