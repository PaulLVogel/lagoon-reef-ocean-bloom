...
export class MainScene extends Phaser.Scene {
  ...
  private ensureFantasyBackground() {
    const KEY = "fantasyBackground";
    const SIZE = 256;
    if (!this.textures.exists(KEY)) {
      ...
      g.fillStyle(0x16301a, 1);
      for (let row = 0; row < 4; row++) {
        for (let col = 0; col < 4; col++) {
          g.fillRect(col * 64 + 4, row * 64 + 4, 56, 56);
        }
      }
      g.lineStyle(2, 0x224422, 0.85);
      for (let i = 0; i <= 4; i++) {
        const p = i * 64;
        g.lineBetween(0, p, SIZE, p);
        g.lineBetween(p, 0, p, SIZE);
      }
      g.lineStyle(1, 0x333333, 0.35);
      for (let i = 0; i < 4; i++) {
        for (let j = 0; j < 4; j++) {
          g.strokeRect(j * 64 + 8, i * 64 + 8, 48, 48);
        }
      }
    ...
    this.backgroundTileSprite = this.add.tileSprite(0, 0, w, h, KEY);
    this.backgroundTileSprite.setOrigin(0, 0);
    this.backgroundTileSprite.setScrollFactor(0);
    this.backgroundTileSprite.setDepth(-1);
    this.floor = this.backgroundTileSprite;
  }

  syncBackgroundTile() {
    const bg = this.backgroundTileSprite;
    if (!bg) return;
    const cam = this.cameras.main;
    bg.setSize(this.scale.width || cam.width, this.scale.height || cam.height);
    bg.tilePositionX = cam.scrollX;
    bg.tilePositionY = cam.scrollY;
  }
