...
export class SnakePlayer {
  ...
  constructor(
    ...
    this.snout = scene.add.triangle(HEAD_RADIUS * 0.55, 0, 0, -7, 14, 0, 0, 7, COLOR.snout);
    ...
    for (let i = 0; i < segmentCount; i++) this.addArmedSegment(randomWeaponType());
    this.layoutSegments();
  }
  ...
  nearestChasePoint(x: number, y: number): { x: number; y: number } {
    let bx = this.head.x, by = this.head.y;
  ...
  canMerge(type: WeaponType) { return this.weapons.some((w) => w.type === type && w.tier < WEAPON_TIER_CAP); }
  mergePreviewTier(type: WeaponType) {
    const match = this.weapons.filter((w) => w.type === type && w.tier < WEAPON_TIER_CAP).sort((a, b) => a.tier - b.tier)[0];
  ...
  addArmedSegment(type?: WeaponType) { this.addSegment(); this.armSegment(this.segments.length - 1, type ?? randomWeaponType()); }

  grantWeapon(type: WeaponType): "merged" | "added" {
    const existing = this.weapons.filter((w) => w.type === type && w.tier < WEAPON_TIER_CAP).sort((a, b) => a.tier - b.tier)[0];
  ...
  fullHeal() {
    this.heal(0);
    for (const s of this.body) {
      s.hp = s.maxHp; s.isActive = true; s.lastHit = 0;
      s.segmentSprite.clearTint(); s.segmentSprite.setTint(s.baseTint);
    }
  }
  ...
  private spawnSegment() {
    ...
    healthBar.setVisible(false); healthBar.setActive(false);
  ...
  private ensureSegTexture() {
    ...
    g.setVisible(false); g.fillStyle(0xffffff, 1); g.fillCircle(SEGMENT_RADIUS, SEGMENT_RADIUS, SEGMENT_RADIUS);
  ...
  private greyOut(s: BodySegment) {
    ...
    s.healthBar.clear(); s.healthBar.setVisible(false);
  ...
  private attachMount(index: number, type: WeaponType) {
    const seg = this.segments[index]; const state = this.body[index];
    if (!seg || seg.getData("armed")) return;
  ...
  private paintTier(w: Weapon) {
    if (w.segmentIndex === HEAD_WEAPON_INDEX) { this.head.setScale(1 + (w.tier - 1) * 0.08); return; }
  ...
  private tickWeapons(now: number, enemies: EnemyScan[]) {
    ...
    for (const w of this.weapons) {
      ...
      if (now - w.lastFired < weaponCooldown(w, this.cooldownMul)) continue;
      const origin = this.originFor(w); if (!origin) continue;
      const dmg = weaponDamage(w, this.damageMul); const range = weaponRange(w);
      if (w.type === "aura") {
        this.fire({ kind: "aura", x: origin.x, y: origin.y, vx: 0, vy: 0, damage: dmg, color: COLOR.aura, radius: range });
        w.lastFired = now; continue;
      }
      if (w.type === "mine_layer") {
        this.fire({ kind: "mine", x: origin.x, y: origin.y, vx: 0, vy: 0, damage: dmg, color: COLOR.mine, radius: 22 + w.tier * 6 });
        w.lastFired = now; continue;
      }
      ...
      if (w.type === "single_shot") {
        this.fire({ kind: "bullet", x: origin.x + ux * 16, y: origin.y + uy * 16, vx: ux * SHOT_SPEED, vy: uy * SHOT_SPEED, damage: dmg, color: COLOR.singleShot, radius: 4 });
      } else if (w.type === "cone_burst") {
        const pellets = 3 + Math.max(0, w.segmentIndex) % 3, mid = (pellets - 1) / 2;
        ...
        this.fire({ kind: "bullet", x: origin.x + ux * 18, y: origin.y + uy * 18, vx: ux * RAIL_SPEED, vy: uy * RAIL_SPEED, damage: dmg, color: COLOR.rail, radius: 3, pierce: true });
      } else if (w.type === "chain_lightning") {
        this.fire({ kind: "chain", x: origin.x, y: origin.y, vx: ux * 640, vy: uy * 640, damage: dmg, color: COLOR.chain, radius: 6, bounces: CHAIN_BOUNCES, bounceRadius: CHAIN_RADIUS });
      } else {
  ...
  private layoutSegments() {
    ...
    for (let i = 0; i < this.segments.length; i++) {
      const idx = Math.max(0, last - (i + 1) * this.historyStride);
      const pos = hist[idx]!; const seg = this.segments[i]!;
